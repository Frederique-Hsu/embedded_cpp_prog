...
class dynamic_array
{
  public:
    ...
    // Constructors.
    dynamic_array() : N(0U), elems(allocator_type().allocate(1U))
    {
        elems[0U] = value_type();
    }

    dynamic_array(size_type count) : N(count), elems(allocator_type().allocate((std::max)(size_type(1U), N)))
    {
        if (N > size_type(0U))
        {
    ...
    dynamic_array(size_type count, const value_type &v, const allocator_type &a = allocator_type())
        : N(count), elems(allocator_type(a).allocate((std::max)(size_type(1U), N)))
    {
        if (N > size_type(0U))
        {
    ...
    dynamic_array(const dynamic_array &other)
        : N(other.size()), elems(allocator_type().allocate((std::max)(size_type(1U), N)))
    {
        if (N > size_type(0U))
        {
    ...
    dynamic_array(input_iterator first, input_iterator last, const allocator_type &a = allocator_type())
        : N(static_cast<size_type>(std::distance(first, last))),
    ...
    dynamic_array(std::initializer_list<T> lst, const allocator_type &a = allocator_type())
        : N(lst.size()), elems(allocator_type(a).allocate((std::max)(size_type(1U), N)))
    {
        if (N > size_type(0U))
        {
    ...
    reference back()
    {
        return ((N > size_type(0U)) ? elems[N - 1U] : elems[0U]);
    }
...
bool operator<(const dynamic_array<T, alloc> &lhs, const dynamic_array<T, alloc> &rhs)
{
    ...
    else
    {
        ...
        else
        {
            ...
            return std::lexicographical_compare(lhs.begin(), lhs.begin() + count, rhs.begin(), rhs.begin() + count);
        }
    }
}
...
template <typename T, typename alloc> void swap(dynamic_array<T, alloc> &x, dynamic_array<T, alloc> &y)
{
    x.swap(y);
}
} // namespace util
