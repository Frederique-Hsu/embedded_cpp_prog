...
namespace util
{
    ...
    template <typename T, const std::size_t N> inline constexpr std::size_t countof(T (&c_array)[N])
    {
    ...
    template <typename T, const std::size_t N> inline std::size_t countof(T (&c_array)[N])
    {
        static_assert(N > std::size_t(0U), "Sorry, util::countof requires an array size larger than zero.");
